...
def train_model(model, opt):
    ...
    for epoch in range(opt.epochs):
        ...
        if opt.checkpoint > 0:
            torch.save(model.state_dict(), f'{import_path}/model_weights')
...
def main():
    ...
    parser.add_argument('-save_folder_name', type=str, default=f'{import_path}/train_vae_nl')
    ...
    if not os.path.isdir('{}'.format(f'{import_path}/')):
        os.mkdir('{}'.format(f'{import_path}/'))
    ...
    opt.train = create_dataset(opt, SRC, TRG, PROP, tr_te='tr',test_mode=True)
