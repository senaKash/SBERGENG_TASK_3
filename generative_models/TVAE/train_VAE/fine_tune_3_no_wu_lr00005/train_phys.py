...
def train_model(model, opt):
    ...
    for epoch in range(opt.epochs):
        ...
        for i, batch in enumerate(opt.train):
            ...
            c_1, c_2, c_3 = torch.tensor([float(np.random.choice([0, 1])) for s in range(batch.batch_size)]), \
                torch.tensor([float(np.random.choice([0, 1])) for s in range(batch.batch_size)]), \
                torch.tensor([float(np.random.choice([0, 1])) for s in range(batch.batch_size)])
            cond = torch.stack(
                [c_1, c_2, c_3]).transpose(0, 1).to(
                'cuda')
...
def main():
    ...
    parser.add_argument('-load_weights', type=str, default=f'{import_path}/old_weights')
    parser.add_argument('-create_valset', action='store_true')
    parser.add_argument('-floyd', action='store_true')
    parser.add_argument('-checkpoint', type=int, default=20)

    parser.add_argument('-src_data', type=str,
                        default=f'{data_path}/CCDC_fine-tuning/data/physic/molecules/database_CCDC_train.txt')
    parser.add_argument('-src_data_te', type=str,
                        default=f'{data_path}/CCDC_fine-tuning/data/physic/molecules/database_CCDC_test.txt')
    parser.add_argument('-trg_data', type=str,
                        default=f'{data_path}/CCDC_fine-tuning/data/physic/molecules/database_CCDC_train.txt')
    parser.add_argument('-trg_data_te', type=str,
                        default=f'{data_path}/CCDC_fine-tuning/data/physic/molecules/database_CCDC_test.txt')


    parser.add_argument('-save_folder_name', type=str, default=f'{import_path}/train_vae')
    ...
    if opt.floyd is not None:
        if not os.path.isdir('weights'):
            os.mkdir('weights')
        pickle.dump(SRC, open('weights/SRC_re.pkl', 'wb'))
        pickle.dump(TRG, open('weights/TRG_re.pkl', 'wb'))
    clssr = Classifier()
